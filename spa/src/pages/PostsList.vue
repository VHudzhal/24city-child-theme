...
export default {
      ...
      if ( this.wpPosts ) {
        ...
        const filteredPosts = this.wpPosts.filter( post => {
          return (
            ...
            post.excerpt.rendered.match( pattern )
          );
        });
        ...
        if ( this.appFilters && this.appFilters.length ) {
          return filteredPosts.filter( post =>
            post.terms.rendered.some( terms =>
  ...
  methods: {
    ...
    getNewsPosts(){
      axios.get('https://24-city.in.ua/wp-json/wp/v2/posts')
        .then((response) => {
    ...
    async getPosts( route = 'posts', namespace = 'wp/v2' ) {
      try {
        ...
        const response = await axios(
          `${restURL}/${namespace}/${route}?per_page=10&page=1`
        );
        ...
        for ( let page = 2; page <= wpTotalPages && 10 >= page; page++ ) {
          ...
          promises.push(
            axios(
              `${restURL}/${namespace}/${route}?per_page=10&page=${page}`
            )
          );
        }
